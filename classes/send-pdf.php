...
';
    ...
    function wpcf7pdf_name_pdf($id)
    {
        ...
        if (isset($meta_values["pdf-name"]) && !empty($meta_values["pdf-name"])) {
            ...
            //Change pdf name here
            if (preg_match('/^\[.*\]$/', $meta_values["pdf-name"])) {

                preg_match('/^\[(.*)\]$/', $meta_values["pdf-name"], $matches);

                // Get current WPCF7 instance
                ...
                if ($submission) {
                    ...
                    if (isset($posted_data[$matches[1]])) {
                        $your_name = $posted_data[$matches[1]];
                        $namePDF = $your_name;
                    }
                }
            } else {
    ...
    function wpcf7pdf_send_pdf($contact_form)
    {
        ...
        if ($submission) {
            ...
            if (isset($meta_values['generate_pdf']) && !empty($meta_values['generate_pdf'])) {
                ...
                // On génère le PDF (Is is enable PDF)
                ...
                if (isset($meta_values["disable-csv"]) && $meta_values['disable-csv'] == 'false') {
                    ...
                    if (isset($meta_values["csv-separate"]) && !empty($meta_values["csv-separate"])) {
                        $csvSeparate = esc_html($meta_values["csv-separate"]);
                    } else {
                        $csvSeparate = ',';
                    }
    ...
    static function wpcf7pdf_deactivation()
    {
        ...
        if (get_option('wpcf7pdf_path_temp')) {
...
?>
                ...
                <script type='text/javascript'>
                    document.addEventListener('wpcf7mailsent', function(event) {
                        <?php if (isset($redirectPDF)) {
                            echo $redirectPDF;
                        } ?>
                        <?php if ((isset($meta_values['page_next']) && is_numeric($meta_values['page_next']))) {
                            echo $js;
                        } ?>
                    }, false);
                </script>
